    ...
    def get_data_list(self):
        return [*self.title, *self.location, *self.department, *self.salary_range, *self.company_profile,
                *self.description, *self.requirement, *self.benefit, self.telecommuting, self.has_company_logo,
                self.has_question, *self.employment_type, *self.required_experience,
                *self.required_education, *self.industry, *self.function]

    def get_target_list(self):
        if int(self.fraudulent) == 0:
            return [1.0, 0.0]  # Real Job Posting
        else:
            return [0.0, 1.0]  # Fake Job Posting

    def get_target(self):
        return int(self.fraudulent)  # Converts Fraudulent to an integer 0/1

    def get_data_tensor(self):
        return torch.tensor(self.get_data_list())  # Converts Job data list into pytorch tensor


class JobPostingsDataset(torch.utils.data.Dataset):  # used to load and manage data efficiently when training models
    ...
    def prepare_all_text_vectorizers(self):
        ...
        all_company_profiles_list = [self.preprocess_text(job_posting.company_profile) for job_posting in
                                     self.job_postings_list]
        ...
        all_descriptions_list = [self.preprocess_text(job_posting.description) for job_posting in
                                 self.job_postings_list]
        ...
        all_requirements_list = [self.preprocess_text(job_posting.requirement) for job_posting in
                                 self.job_postings_list]
        ...
        all_employment_types_list = [self.preprocess_text(job_posting.employment_type) for job_posting in
                                     self.job_postings_list]
        ...
        all_required_experiences_list = [self.preprocess_text(job_posting.required_experience) for job_posting in
                                         self.job_postings_list]
        ...
        all_required_educations_list = [self.preprocess_text(job_posting.required_education) for job_posting in
                                        self.job_postings_list]
    ...
    def preprocess_text(text):
        ...
        symbols = re.findall("(?::|;|=)(?:-)?(?:\)|\(|D|P)", text)  # Find Emojis
        text = (re.sub("[\W]+", " ", text.lower()) + " ".join(symbols).replace("-", ""))  # Convert to lowercase
        return text
    ...
    def __getitem__(self, index):
        if index in self.vectorized_job_postings_dict:
            ...
            vectorized_job_posting.location = \
            self.location_vectorizer.transform([vectorized_job_posting.location]).toarray()[0]
            vectorized_job_posting.department = \
            self.department_vectorizer.transform([vectorized_job_posting.department]).toarray()[0]
            vectorized_job_posting.company_profile = \
            self.company_profile_vectorizer.transform([vectorized_job_posting.company_profile]).toarray()[0]
            vectorized_job_posting.description = \
            self.description_vectorizer.transform([vectorized_job_posting.description]).toarray()[0]
            vectorized_job_posting.requirement = \
            self.requirement_vectorizer.transform([vectorized_job_posting.requirement]).toarray()[0]
            vectorized_job_posting.benefit = \
            self.benefit_vectorizer.transform([vectorized_job_posting.benefit]).toarray()[0]
            vectorized_job_posting.employment_type = \
            self.employment_type_vectorizer.transform([vectorized_job_posting.employment_type]).toarray()[0]
            vectorized_job_posting.required_experience = \
            self.required_experience_vectorizer.transform([vectorized_job_posting.required_experience]).toarray()[0]
            vectorized_job_posting.required_education = \
            self.required_education_vectorizer.transform([vectorized_job_posting.required_education]).toarray()[0]
            vectorized_job_posting.industry = \
            self.industry_vectorizer.transform([vectorized_job_posting.industry]).toarray()[0]
            vectorized_job_posting.function = \
            self.function_vectorizer.transform([vectorized_job_posting.function]).toarray()[0]
            if len(list(vectorized_job_posting.salary_range[0].split("-"))) == 2:
                try:
                    vectorized_job_posting.salary_range = tuple(
                        map(int, vectorized_job_posting.salary_range[0].split("-")))
                except:
